...
func NewPNService() *PushNotificationService {
	...
	pns := &PushNotificationService{
		...
		GeminiResponse:          make(chan *models.GeminiResponse, 4), // for lowering usage limit
	}
...
func (pns *PushNotificationService) StartService() {
	...
	for {
		...
		case geminiResPayload := <-pns.GeminiResponse:
			slog.Info("payload recieved", "type", geminiResPayload.Type)

			// $ handler invokes pns service's method to
			// constructing the payload for publishing data
			payload := &ClientNotifyPayload{
				SenderID:   geminiResPayload.SenderID, //* client sending req would be the one who would be recieving the data
				RecieverID: geminiResPayload.GeminiID, //* need to think about it
				Type:       "denverai_prompt",         //* need to explicitly mark type for publishing and routing
				Content:    geminiResPayload.AIResponse,
				CreatedAt:  time.Now(),
				RoomID:     0,
			}

			if pns.Hub != nil {
				// if hub is available publish to the exchange
				err := pns.Hub.BrokerInterface.PublishEvents(payload.SenderID, payload) //& this time we would be stamping delivery on "senderID" as he would be the reciever
				if err != nil {
					slog.Error("failed to publish and stamp for the ai prompt delivery", "error", err)
					return
				}
			}
			...
			slog.Info("payload recieved in the pns service for publishing to the exchange",
				slog.Group("denverai",
					slog.Uint64("senderID", uint64(payload.SenderID)),
					slog.String("reciever", "denverai handler"),
					slog.String("notification_type", payload.Type),
				),
				slog.Group("redirecting payload to the consumer",
					slog.Time("requested_at", time.Now())),
