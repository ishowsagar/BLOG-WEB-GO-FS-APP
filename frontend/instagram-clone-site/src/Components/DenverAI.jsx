...
export default function DenverAI() {
  ...
  const handleSend = (textToSend) => {
    ...
    setTimeout(() => {
      ...
      // Look for predefined mock answer, otherwise generate generic response
      if (MOCK_ANSWERS[text]) {
        replyText = MOCK_ANSWERS[text];
      } else {
        replyText = `Hello! I've received your prompt: "${text}". \n\nThis is a premium frontend preview of **Denver AI**. In the next phase, we'll hook up this beautiful interface to the backend Gemini API to generate real, dynamic responses using the \`gemini-2.5-flash\` model! \n\n*Tip: Try clicking any of the suggested prompts in the sidebar to see how I format responses.* ✨`;
      }
      ...
      <div className="denver-sidebar denver-glass">
        <div className="denver-brand-section">
          <div className="denver-ai-logo-outer">
            <svg className="denver-prompt-icon" style={{ width: "32px", height: "32px", fill: "white" }} viewBox="0 0 24 24">
          ...
          <p className="denver-brand-tagline">
            Your glassmorphic assistant for content creation, coding queries, and profile design.
          </p>
        </div>
        ...
        <div className="denver-status-panel">
          ...
          <div className="denver-status-row">
            ...
            <span className="denver-status-value" style={{ color: "#3b82f6" }}>Secure WebSocket</span>
          </div>
        </div>
      </div>
        ...
        <div className="denver-chat-header">
          <div className="denver-chat-header-info">
            ...
            <div className="denver-chat-header-details">
              <span className="denver-chat-header-title">Denver Intelligent Core</span>
          ...
          {messages.length === 0 ? (
            <div className="denver-welcome-container">
              ...
              <p className="denver-welcome-desc">
                Welcome to Denver AI Core. Generate creative blog drafts, refine your bio content, or ask questions instantly. Pick a quick prompt below or start typing.
              </p>
              ...
              <div className="denver-capabilities-grid">
                <div className="denver-cap-card">
                  ...
                  <span className="denver-cap-text">Generate ready-to-post drafts with hashtags and tags.</span>
                </div>
                <div className="denver-cap-card">
                  <span className="denver-cap-icon">💡</span>
                  <span className="denver-cap-label">Optimizations</span>
                  <span className="denver-cap-text">Refine your profile descriptions and bios effortlessly.</span>
                </div>
                <div className="denver-cap-card">
                  <span className="denver-cap-icon">⚡</span>
                  <span className="denver-cap-label">Instant Replies</span>
                  <span className="denver-cap-text">Powered by the lightning-fast Gemini 2.5 Flash model.</span>
                </div>
                <div className="denver-cap-card">
                  <span className="denver-cap-icon">🧩</span>
                  <span className="denver-cap-label">Explain Concepts</span>
                  <span className="denver-cap-text">Ask complex programming or configuration questions.</span>
                </div>
              </div>
            </div>
          ) : (
            messages.map((msg) => (
              <div key={msg.id} className={`denver-msg-row ${msg.sender === "user" ? "is-user" : "is-ai"}`}>
