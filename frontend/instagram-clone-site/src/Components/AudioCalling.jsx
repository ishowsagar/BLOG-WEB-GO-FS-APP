...
const AudioCalling = forwardRef(({ 
  ...
  const handleHangup = () => {
    ...
    const remoteAudioElement = document.getElementById("remote-hidden-audio-element");
  ...
  const acceptCall = async () => {
    try {
      ...
      peerConnection.current.ontrack = (e) => {
        ...
        let remoteHiddenAudioElement = document.getElementById("remote-hidden-audio-element");
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeNotifications(async (audioPayload) => { //** interceptor recieved the payload on onmessage
      if (!audioPayload) return;

      switch (audioPayload.type) { //& when call 'offer' is recieved <- for cal connection => reciever get that request with peerID being the senderID as sender is one who is sending call request
        case "offer": {
        ...
        case "answer": { // &when call is either approved or not <- 'answering' call => reciever gets 'answer''s audio_payload
          if (peerConnection.current) {
            await peerConnection.current.setRemoteDescription(
        ...
        case "hangup": { //& handle hangup state <- when peerID becomes unavailable -> hangup the call // asking reciever client to hang up the calling connection and media devices as peer has been disconnected
          handleHangup();
          break;
        }
      }
  ...
  }, [subscribeNotifications, passedCurrentUserID, incomingOfferSdp]); //only re render calling service if current userID get alernated unexpectedly
  ...
  const IntialiseCalling = async (recieverID) => {
    ...
    try {
      ...
      localStream.current = await navigator.mediaDevices.getUserMedia({ // mediaDevices are readyOnly props <- read what client has provided to the browser
        audio: true,
        video: false,
      });
      ...
      peerConnection.current.onicecandidate = (e) => {
        ...
        if (e.candidate) {
          ...
          if (sendNotifications) { // if ws.send is available
            const payload = {
              sender_id: Number(passedCurrentUserID),
      ...
      peerConnection.current.ontrack = (e) => {
        const remoteAudioStream = e.streams[0]; // on track property gives us remotely recieved stream <- in streams array being the 0th as first element being the recieved stream
        let remoteHiddenAudioElement = document.getElementById("remote-hidden-audio-element"); // ohhh, this would be played in hidden side but hearble track
  ...
  return (
    <div className="audiocall-overlay">
      <div className="audiocall-card">
        ...
        <h3 className="audiocall-name">
          User #{peerLabel}
        </h3>
        
        <p className={`audiocall-status ${callState === "active" ? "is-active" : "is-pending"}`}>
          {getStatusText()}
        </p>

        {/* Dynamic call timer */}
        {callState === "active" && (
          <div className="audiocall-timer">
            {getFormattedTime()}
          </div>
        )}
        ...
        {callState === "hangup" && finalDuration && (
          <div className="audiocall-duration">
            Call Duration: <span className="audiocall-duration-value">{finalDuration}</span>
          </div>
        )}
